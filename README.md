...
- `fishbowl`: every busy request slot is a fish (blue tangs for GPU 0's server, goldfish for GPU 1's) that swims in from off screen when the request starts and out when it finishes. Tokens are fish food: each server's tokens drop from its floating feeding ring as flakes at the real rate (one per 35 tokens), and its fish chase and eat them. Light rays, caustics, seaweed, a bubbler and a very slow snail
